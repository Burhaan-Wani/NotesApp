...
def Signup(request):
    ...
    if request.method == "POST":
        ...
        if check_existing_user:
            ...
            return HttpResponse('Minimum 8+ chars, atleast 1 Uppercase, 1 Lowercase, 1 Number, and 1 special Chars.')
        
        elif email == '' or uname == '' or pwd == '' or cpwd == '':
            return HttpResponse("fields can't be left blank.")

        elif (email_regEx.search(email)) and (uname_regEx.search(uname)) and pwd == cpwd and (pwd_regEx.search(pwd)) and request.POST.get('otp'):
            otp = request.POST.get('otp')
            if int(otp) == int(request.session['random']):
                newUser = signup.objects.create(username=uname,email=email,password=pwd,status='inactive')
                newUser.save()
        ...
        elif (email_regEx.search(email)) and (uname_regEx.search(uname)) and pwd == cpwd and (pwd_regEx.search(pwd)):
...
def SendOtp(request):
    ...
    email = EmailMessage(  
            "hello", mail_subject ,to= [request.session['email']]
    )
    email.send()
...
def resetOtp(request):
    ...
    email = EmailMessage(  
            "Hi there", mail_subject ,to= [request.session['email']]
    )
    email.send()
...
def forgotPassword(request):
    reset_pwd_regEx = re.compile(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$")
    if request.method == "POST":
        ...
        if request.POST.get('newPassword') and request.POST.get('confirmNewPassword'):
            ...
            if (not request.POST.get('newPassword')) and (not request.POST.get('confirmNewPassword')):
                return HttpResponse("Field's can't be left blank.")

            if reset_pwd_regEx.search(newPassword) and newPassword != confirmNewPassword:
                return HttpResponse("New Password and Confirm New Password do not match.")

            elif (not reset_pwd_regEx.search(newPassword)) and newPassword == confirmNewPassword:
                return HttpResponse("Minimum 8+ chars, atleast 1 Uppercase, 1 Lowercase, 1 Number, and 1 special Chars.")
            
            elif reset_pwd_regEx.search(newPassword) and newPassword == confirmNewPassword :
        ...
        if request.POST.get('email'):
            ...
            for emailData in user:    
                ...
                break
            if len(user)==0:
                return HttpResponse("Couldn't find such email, check your email once again.")
...
def NoteApp(request):
    ...
    if request.method=="POST":
        ...
        if todoTitle and description:
            note = notes.objects.create(title=todoTitle, description=description,user_id=userID)
            note.save()
